...
class StockTransferDetails(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if not picking.purchase_subcontratacion:
            ...
            res = super(StockTransferDetails,
                        self.with_context(
                            from_subcontratacion=True)).default_get(fields)
        ...
        for item in item_ids:
            if 'packop_id' in item and item.get('packop_id', False):
                packop = self.env['stock.pack.operation'].browse(
                    item.get('packop_id'))
...
class StockTransferDetailsItems(models.TransientModel):
    ...
    operation_quantity = fields.Float(
        string='Operation Quantity', related='packop_id.product_qty',
        digits=dp.get_precision('Product Unit of Measure'))
    operation_picking_id = fields.Many2one(
        string='Operation picking', comodel_name='stock.picking',
    ...
    def onchange_quantity(self):
        if (self.quantity and
                self.operation_picking_id.purchase_subcontratacion):
            if self.quantity != self.operation_quantity:
                self.quantity = self.operation_quantity
                return {'warning':
                        {'title': _('Quantity error'),
                         'message': _('Cannot exceed quantity defined in '
