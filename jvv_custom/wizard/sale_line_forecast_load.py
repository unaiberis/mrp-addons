...
class SaleLineForecastLoad(models.TransientModel):
    ...
    sale_id = fields.Many2one(
        comodel_name="sale.order", string="Pedido venta",
    ...
    def load_sales_lines(self):
        ...
        if not forecasts:
            raise exceptions.Warning("No se ha encontrado ninguna previsión de venta para este producto")
        return {
            'view_type': 'form',
            'view_mode': 'tree,form',
    ...
    def onchange_product_id(self):
        if self.product_id and self.partner_id:
            ...
            print ('*** self.product_id.customer_ids: ' + str(self.product_id.customer_ids))
            found = False
            if self.product_id.customer_ids:
                for line in self.product_id.customer_ids:
                    if line.name == self.partner_id:
                        found = True
            if not found:
                raise exceptions.Warning("No se ha encontrado relacion para la venta producto/cliente.")
