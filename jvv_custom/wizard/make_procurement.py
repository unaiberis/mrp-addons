...
class MakeProcurement(models.TransientModel):
    ...
    def make_procurement(self):
        ...
        if (self.env.context.get('mps_line_id', False) and
            result.get('res_model', False) and
                result.get('res_model') == 'procurement.order'):
            ...
            if proc.production_id:
                proc.production_id.origin = u"MPS: {}".format(
                    line.forecast_id.name)
            if not proc.origin:
                proc.origin = u"MPS: {}".format(line.forecast_id.name)
        return result
