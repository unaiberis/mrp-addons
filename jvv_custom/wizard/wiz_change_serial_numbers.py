...
class WizChangeSerialNumbers(models.TransientModel):
    ...
    move_id = fields.Many2one(
        string="Movimiento", comodel_name="stock.move")
    old_serial_numbers = fields.Text(
        string='Números de serie actuales', copy=False)
    new_serial_numbers = fields.Text(
        string='Nuevos números de serie', copy=False)

    @api.model
    def default_get(self, var_fields):
        result = super(
            WizChangeSerialNumbers, self).default_get(var_fields)
    ...
    def change_serial_numbers(self):
        ...
        if (self.move_id.sale_id and self.move_id.production_id and
                self.move_id.production_id.sale_line_id):
            cond = [('sale_order_line', '=',
                     self.move_id.production_id.sale_line_id.id),
