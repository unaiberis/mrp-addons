...
class MrpBomLine(models.Model):
    ...
    my_counter = fields.Integer(
        string="Numeración", readonly="1")

    @api.model
    def create(self, values):
        bom_line = super(MrpBomLine, self).create(values)
    ...
    def automatic_put_consumido_en_in_bom_lines(self):
        ...
        for line in lines:
            cond = [('product_id', '=', line.product_id.id),
                    ...
                    ('raw_material_production_id.product_tmpl_id', '=', line.bom_id.product_tmpl_id.id),
                    ('work_order', '!=', False),
