...
class MrpProduction(models.Model):
    ...
    mo_skewback_date = fields.Date(string="MO Skewback date")
    ...
    ldm_total_cost = fields.Float(
        string="LdM Total Cost", related="bom_id.total_cost", store=True, copy=False
    )
    ldm_num_lines = fields.Integer(
        string="Num. Líneas en LdM", related="bom_id.num_lines", copy=False, store=True
    )
    ...
    def _compute_permission_to_block_manufacture(self):
        ...
        permission = True if self.env.user in mrp_manager_group.users else False
    ...
    def action_assign(self):
        for production in self.filtered(
            ...
            and x.state in ("confirmed", "picking_except")
        ):
            raise exceptions.Warning(
                ("La órden de fabricación: %s, está bloqueada.") % production.name
            )
    ...
    def _calc_production_attachments_jvv(self):
        for production in self:
            ...
            if (
                production.parent_mrp_production_id
                and production.parent_mrp_production_id.product_attachments
            ):
    ...
    def _compute_productions_count(self):
        ...
        for p in self:
            if not p.move_prod_id and p.project_id:
                cond = [("project_id", "=", p.project_id.id), ("id", "!=", p.id)]
    ...
    def _compute_sale_line_dates2(self):
        for p in self.filtered(lambda c: c.parent_mrp_production_id):
            ...
            if (
                p.parent_mrp_production_id.sale_line_id
                and p.parent_mrp_production_id.sale_line_id.requested_date
            ):
            ...
            if (
                p.parent_mrp_production_id.sale_line_id
                and p.parent_mrp_production_id.sale_line_id.confirmed_date
            ):
                sale_line = p.parent_mrp_production_id.sale_line_id
                new_date = _convert_to_local_date(
    ...
    def _compute_parent_commitment_date(self):
        for p in self.filtered(lambda c: c.parent_mrp_production_id):
            if (
                p.parent_mrp_production_id
                and p.parent_mrp_production_id.commitment_date
            ):
    ...
    def _compute_product_code(self):
        for p in self.filtered(
            ...
            and c.product_id
            and c.move_prod_id
            and not c.move_prod_id.raw_material_production_id
        ):
            for s in p.product_id.customer_ids:
                if s.name.id == p.sale_id.partner_id.id:
                    ...
                    break
                if (
                    p.sale_id.partner_id
                    and s.name.id == p.sale_id.partner_id.parent_id.id
                ):
                    p.product_code = s.product_code
                    break
        for p in self.filtered(
            lambda c: not c.sale_id
            and c.move_prod_id
            and c.move_prod_id.raw_material_production_id
            and c.move_prod_id.raw_material_production_id.product_code
        ):
    ...
    def _compute_date_planned_without_hour(self):
        for p in self.filtered(lambda c: c.date_planned):
            p.date_planned_without_hour = fields.Datetime.from_string(
                p.date_planned
            ).date()
            p.date_planned_month = str(
                fields.Datetime.from_string(p.date_planned).month
            )
            p.date_planned_year = fields.Datetime.from_string(p.date_planned).year
            p.date_planned_week = int(
                fields.Datetime.from_string(p.date_planned).strftime("%V")
            )
    ...
    production_line_id = fields.Many2one(
        ...
        comodel_name="mrp.routing.production.line",
        related="routing_id.production_line_id",
        store=True,
    )
    load_time_line = fields.Float(
        string="Load time line", related="routing_id.load_time_line", store=True
    )
    load_time_variable = fields.Float(
        string="Time to load variable",
        compute="_compute_load_time_variable",
        store=True,
    )
    total_line_hours = fields.Float(
        string="Total line hours", compute="_compute_load_time_variable", store=True
    )
    load_time_warehouse = fields.Float(
        string="Load time warehouse",
        store=True,
        related="routing_id.load_time_warehouse",
    )
    load_time_variable_warehouse = fields.Float(
        string="Time variable load warehouse",
        store=True,
        compute="_compute_load_time_variable_warehouse",
    )
    total_warehouse_time = fields.Float(
        string="Total warehouse time",
        store=True,
        compute="_compute_load_time_variable_warehouse",
    )
    date_planned_without_hour = fields.Date(
        string="Date planned", compute="_compute_date_planned_without_hour", store=True
    )
    ...
    date_planned_year = fields.Integer(
        string="Planned year", store=True, compute="_compute_date_planned_without_hour"
    )
    date_planned_week = fields.Integer(
        string="Planned week", store=True, compute="_compute_date_planned_without_hour"
    )
    ...
    purchase_subcontratacion_ids = fields.One2many(
        ...
        inverse_name="mrp_production",
    )
    purchase_subcontratacion_id = fields.Many2one(
        string="Doc. sub.",
        comodel_name="purchase.order",
        compute="_compute_purchase_subcontratacion_id",
        store=True,
    )
    parent_mrp_production_id = fields.Many2one(
        string="Parent production", comodel_name="mrp.production", index=True
    )
    parent_commitment_date = fields.Datetime(
        string="Commitment Date", store=True, compute="_compute_parent_commitment_date"
    )
    product_attachments_jvv = fields.Many2many(
        comodel_name="ir.attachment",
        string="Product attachments",
        compute="_calc_production_attachments_jvv",
        readonly=True,
    )
    ...
    version = fields.Integer(string="BoM Version", related="bom_id.version", store=True)
    ...
    manual_requested_date = fields.Date(string="Fec.Solicitud manual")
    manual_confirmed_date = fields.Date(string="Fec.Confirmada manual")
    ...
    def action_ready(self):
        ...
        for p in self.filtered(
            lambda c: c.move_prod_id
            and c.move_prod_id.raw_material_production_id
            and c.move_prod_id.raw_material_production_id.location_dest_id.id
            == location.id
            and c.move_prod_id.location_id.id
    ...
    def buttom_show_related_productions(self):
        ...
        if not self.move_prod_id:
            cond = [("project_id", "=", self.project_id.id), ("id", "!=", self.id)]
            productions = p_obj.search(cond)
        if self.move_prod_id and self.move_prod_id.raw_material_production_id:
            productions = self.move_prod_id.raw_material_production_id
        if productions:
            return {
                "name": "OFs relacionadas",
                "type": "ir.actions.act_window",
    ...
    def create(self, values):
        ...
        if production.bom_id:
            lines = production.bom_id.bom_line_ids.filtered(lambda x: not x.operation)
            if lines:
    ...
    def write(self, values, update=True, mini=True):
        ...
        result = super().write(values, update=update, mini=mini)
    ...
    def action_confirm(self):
        ...
        sequence = self.env.ref("jvv_custom.move_num_serie_sequence", False)
        for production in self.filtered(lambda c: not c.date_planned):
            raise exceptions.Warning(_("You must enter the scheduled date."))
        ...
        for production in self:
            ...
            lines = production.move_created_ids.filtered(
                lambda l: l.product_id.generate_serial_numbers == "yes"
                and not l.serial_numbers
            )
            for line in lines:
                i = 1
                numbers = ""
                while i <= line.product_uom_qty:
                    ...
                    numbers += serial if not numbers else ", {}".format(serial)
                    i += 1
                if numbers:
                ...
                if (
                    numbers
                    and line.sale_id
                    and line.production_id
                    and line.production_id.sale_line_id
                ):
                    cond = [
                        ("sale_order_line", "=", line.production_id.sale_line_id.id)
                    ]
            ...
            if not production.origin:
                ...
                productions = self.search(cond)
                if productions:
                    productions.write({"parent_mrp_production_id": production.id})
...
class MrpProductionWorkcenterLine(models.Model):
    ...
    def _compute_day(self):
        for line in self.filtered("date_planned"):
            from_date = _convert_to_local_date(line.date_planned, self.env.user.tz)
            line.day = str(from_date.date().weekday())

    @api.depends(
        "hour",
        "production_id",
        "production_id.routing_id",
    ...
    def _compute_total_phase_time(self):
        for line in self:
            ...
            if (
                line.production_id
                and line.production_id.routing_id
                and line.production_id.routing_id.workcenter_lines
            ):
                workcenters = line.production_id.routing_id.mapped(
                    "workcenter_lines"
                ).filtered(
                    lambda l: l.sequence == line.sequence
                    and l.workcenter_id == line.workcenter_id
                )
                if workcenters:
                    for work in workcenters:
                        if work.op_wc_lines:
                            ...
                            if op_wc_lines:
                                boot_time = sum(op_wc_lines.mapped("time_start"))
                                hours += boot_time
                                stopping_time = sum(op_wc_lines.mapped("time_stop"))
                                hours += stopping_time
    ...
    def _calc_workcenter_attachments_jvv(self):
        for line in self:
            ...
            if line.workcenter_id:
                ...
                if attachment:
                    attachments += attachment
            if line.production_id and line.production_id.product_attachments_jvv:
    ...
    total_phase_time = fields.Float(
        string="Total phase time", compute="_compute_total_phase_time", store=True
    )
    boot_time = fields.Float(
        string="Boot time", compute="_compute_total_phase_time", store=True
    )
    boot_time = fields.Float(
        string="Boot time", compute="_compute_total_phase_time", store=True
    )
    stopping_time = fields.Float(
        string="Stopping time", compute="_compute_total_phase_time", store=True
    )
    date_planned_month = fields.Selection(
        [
            ("1", _("January")),
    ...
    workcenter_attachments_jvv = fields.Many2many(
        comodel_name="ir.attachment",
        string="Product attachments",
        compute="_calc_workcenter_attachments_jvv",
        readonly=True,
    )
    origin = fields.Char(
        string="Origen producción",
