...
class ProductTemplate(models.Model):
    ...
    @api.depends(
        "template_cost_price_history_ids",
        "template_cost_price_history_ids.datetime",
        "template_cost_price_history_ids.cost",
    )
    def _compute_last_date_cost(self):
        for template in self.filtered(lambda l: l.template_cost_price_history_ids):
            c = max(template.template_cost_price_history_ids, key=lambda x: x.datetime)
            if c:
    ...
    @api.depends(
        "procurement_sale_forecast_line_ids",
    ...
    @api.depends(
        "procurement_sale_forecast_line2_ids",
    ...
    fix_negative_stock = fields.Boolean(string="Fix negative stock", default=False)
    template_cost_price_history_ids = fields.One2many(
        string="Cost price history",
        comodel_name="product.price.history",
        inverse_name="product_template_id",
    )
    last_date_cost = fields.Datetime(
        string="Last date cost price", compute="_compute_last_date_cost", store=True
    )
    last_date_manual_cost = fields.Datetime(
        string="Last date manual cost price",
        store=True,
    ...
    mrp_bom_ids = fields.One2many(
        string="MRP BoMs", comodel_name="mrp.bom", inverse_name="product_tmpl_id"
    )
    ...
    forecast2_ids = fields.Many2many(
        ...
        copy=False,
        compute="_compute_forecast2_ids",
        store=True,
    )
    robot = fields.Boolean(string="Robot", default=False)
    ...
    def automatic_fix_product_negative_stock(self):
        ...
        for template in templates:
            try:
                data = [
                    "product_id",
                    "delete_negative_quants",
                    "lot_id",
                    "new_quantity",
                    "location_id",
                ]
                res = wiz_obj.with_context(
                    active_id=template.id, active_model="product.template"
                ).default_get(data)
