...
class ProjectTask(models.Model):
    ...
    project_from_mrp = fields.Boolean(
        string='Project from mrp', compute='_compute_project_from_mrp',
        store=True)
    ...
    def create(self, values):
        ...
        if 'project_id' in values and values.get('project_id', False):
            ...
            if production:
                ...
                if line:
                    name = u"{} - {} - {}".format(
                        line.order_id.name, production.product_id.name,
                        production.product_id.product_manager.name)
                    values['name'] = name
                if not line:
                    cond = [('id', '!=', production.id),
                            ('analytic_account_id', '=',
                             production.analytic_account_id.id)]
                    production = production_obj.search(cond, limit=1)
                    if production:
                        cond = [('mrp_production_id', '=', production.id)]
                        line = self.env['sale.order.line'].search(cond, limit=1)
    ...
    def unlink(self):
        for type in self:
            raise exceptions.Warning(
                _("You cannot delete the task stage %s, what you can do is "
                  "deactivate it.") % type.name)
