...
class StockLocation(models.Model):
    ...
    run_mrp_scheduler = fields.Boolean(string="Usar en planificador MRP", default=False)


class StockLocationRoute(models.Model):
    _inherit = "stock.location.route"

    assemtronic_product = fields.Boolean(string="Assemtronic product", default=False)
...
class StockProductionLot(models.Model):
    ...
    qty = fields.Float(string="Quantity", compute="_compute_qty", store=True)
    manufacturer_id = fields.Char(string="ID Manufacturer", copy=False)

    @api.model
    def create(self, values):
        if "default_manufacturer_id" in self.env.context and self.env.context.get(
            "default_manufacturer_id", False
        ):
            values["manufacture_id"] = self.env.context.get("default_manufacturer_id")
        lot = super().create(values)
        return lot
...
class StockPicking(models.Model):
    ...
    def _compute_date_without_hour(self):
        for picking in self:
            ...
            if picking.date:
                new_date = _convert_to_local_date(picking.date, self.env.user.tz)
    ...
    def _compute_recorde_serial_number(self):
        for picking in self:
            ...
            if (
                picking.purchase_subcontratacion
                and picking.purchase_subcontratacion.mrp_production
            ):
                ...
                for s in move.product_id.customer_ids:
                    if s.name.id == picking.partner_id.id and s.product_code:
                        code = (
                            s.product_code
                            if not code
                            else "{}, {}".format(code, s.product_code)
                        )
                        break
                    if (
                        picking.partner_id
                        and s.product_code
                        and s.name.id == picking.partner_id.parent_id.id
                    ):
                        code = (
                            s.product_code
                            if not code
    ...
    sale_line_confirmed_date = fields.Date(string="Fec.Confirm.línea venta", store=True)
    product_code = fields.Char(
        string="Código producto empresa", compute="_compute_product_code", store=True
    )
    date_done_without_hour = fields.Date(
        string="Transfer date", compute="_compute_date_done_without_hour", store=True
    )
    ...
    shipping_address_in_pickings = fields.Boolean(
        string="Request shipping address in pickings",
        store=True,
    ...
    date_without_hour = fields.Date(
        string="Fecha creación", compute="_compute_date_without_hour", store=True
    )
    min_date_without_hour = fields.Date(
        string="Fecha prevista", compute="_compute_min_date_without_hour", store=True
    )
    ...
    def do_enter_transfer_details(self, cr, uid, picking, context=None):
        ...
        if not picking:
            result = super().do_enter_transfer_details(
        ...
        if p.state == "assigned":
            ...
            for product in products:
                ...
                my_move = p.move_lines.filtered(
                    lambda x: x.product_id.id == product.id and x.state == "confirmed"
                )
                if len(my_move) == 0:
                    my_move = p.move_lines.filtered(
                    ...
                    if len(my_move) == 0:
                        raise exceptions.Warning(
                            _(
                                "No se ha encontrado el movimiento para el "
                                "producto: %s"
                            )
                            % product.name
                        )
                if len(my_move) > 1:
                    raise exceptions.Warning(
                        _(
                            "Se ha encontrado mas de un movimiento para el "
                            "producto: %s"
                        )
                        % product.name
                    )
                for ope in my_operations:
                    if ope.especial_lot and (
                        ope.reserved_quant_for_especial_lot_ids
                        or ope.free_quant_for_especial_lot_ids
                    ):
                        for q in ope.reserved_quant_for_especial_lot_ids:
                            if (
                                q.reservation_id
                                and q.reservation_id.state == "assigned"
                            ):
                                move_obj.do_unreserve(
                                    cr, uid, q.reservation_id.ids, context=context
                                )
                    ...
                    if qty_to_found > 0 and ope.free_quant_for_especial_lot_ids:
                        for q in ope.free_quant_for_especial_lot_ids:
                            my_qty = 0
                            if qty_to_found > 0 and q.qty <= qty_to_found:
                                qty_to_found -= q.qty
                            ...
                            if my_qty > 0:
                                ...
                                line = wiz_line_obj.create(cr, uid, vals)
                    if (
                        not ope.free_quant_for_especial_lot_ids
                        and not ope.free_quant_for_especial_lot_ids
                    ):
                        quant = False
                        available_quant = self.pool.get("stock.quant").search(
                            ...
                            [
                                ...
                                ("lot_id", "=", ope.lot_id.id),
                                ("location_id", "child_of", ope.location_id.id),
        ...
        if p.state in ("assigned", "partially_available"):
            for ope in p.pack_operation_ids:
                ...
                opes_ids = self.pool.get("stock.pack.operation").search(cr, uid, cond)
                if opes_ids:
                ...
                if (opes_qty + ope.product_qty) > moves_qty:
                    raise exceptions.Warning(
                        _(
                            "For the product %s, are needed one amount of %s, "
                            "and in operations have been introduced %s."
                        )
                        % (ope.product_id.name, moves_qty, opes_qty + ope.product_qty)
                    )
            result = super().do_enter_transfer_details(
                cr, uid, picking, context=context
            )
    ...
    def do_transfer(self):
        ...
        for picking in self.filtered(
            lambda c: c.group_id
            and c.picking_type_id
            and c.picking_type_id.code == "outgoing"
        ):
        ...
        for picking in self.filtered(
            lambda c: c.picking_type_id and c.picking_type_id.code == "incoming"
        ):
            purchases = self.env["purchase.order"]
            for move in picking.move_lines.filtered(lambda c: c.purchase_line_id):
                if (
                    move.purchase_line_id.order_id
                    and move.purchase_line_id.order_id not in purchases
                ):
    ...
    def action_assign(self):
        if self.purchase_subcontratacion:
            result = super(
                StockPicking, self.with_context(no_delete_operations=True)
            ).action_assign()
        else:
    ...
    def _prepare_pack_ops(self, picking, quants, forced_qties):
        result = super()._prepare_pack_ops(
            picking, quants, forced_qties
        )
        if result:
            for line in result:
                lines = picking.move_lines.filtered(
                    lambda x: x.product_id.id == line.get("product_id")
                    and x.location_dest_id.id == line.get("location_dest_id")
                    and x.location_id == line.get("location_id")
                    and x.product_uom_qty == line.get("product_qty")
                )
...
class StockMove(models.Model):
    ...
    def _compute_committed_date(self):
        for move in self.filtered(
            lambda c: c.procurement_id and c.procurement_id.sale_line_id
        ):
    ...
    def _compute_partner_product_code(self):
        for move in self.filtered(lambda c: c.picking_id and c.picking_type_id):
            line = False
    ...
    def _compute_assemtronic_product(self):
        for move in self.filtered(lambda c: c.product_id and c.product_id.route_ids):
    ...
    def _compute_lots_description(self):
        for move in self:
            ...
            for lot in move.lot_ids:
                desc = lot.name if not desc else "{}, {}".format(desc, lot.name)
                if lot.ref:
                    desc_bin = (
                        lot.ref if not desc_bin else "{}, {}".format(desc_bin, lot.ref)
                    )
            if desc:
    ...
    production_product_id = fields.Many2one(
        ...
        compute="_compute_production_product_id",
        store=True,
    )
    ...
    sale_id = fields.Many2one(
        string="Sale order",
        commodel_name="sale.order",
        related="production_id.sale_id",
        store=True,
    )
    assemtronic_product = fields.Boolean(
        string="Assemtronic product", compute="_compute_assemtronic_product", store=True
    )
    lots_description = fields.Char(string="Lots", compute="_compute_lots_description")
    bins_description = fields.Char(string="BINs", compute="_compute_lots_description")
    bar_code_status = fields.Selection(
        string="Reader bar code status",
        selection=[
            ("no", _("No control")),
            ("nocompleted", "No completed"),
            ("completed", "Completed"),
        ],
        compute="_compute_bar_code_status",
        store=True,
    )
    ...
    qty_locked = fields.Float(
        string="Cantidad bloqueada", related="product_id.qty_locked"
    )
    lot_locked_ids = fields.Many2many(
        string="Lotes bloqueados",
    ...
    @api.depends(
        ...
        "picking_id",
        "picking_id.purchase_subcontratacion",
    ...
    def _compute_bar_code_status(self):
        for move in self.filtered(
            lambda c: not c.picking_id
            or (c.picking_id and not c.picking_id.purchase_subcontratacion)
        ):
            move.bar_code_status = "no"
        for move in self.filtered(
            lambda c: c.picking_id and c.picking_id.purchase_subcontratacion
        ):
            if move.product_id.type != "consu":
                if (
                    not move.reserved_quant_ids
                    or not move.picking_id.pack_operation_ids
                ):
                ...
                if move.lot_ids:
                    ...
                    operations = move.picking_id.pack_operation_ids.filtered(
                        lambda c: c.product_id.id == move.product_id.id and not c.lot_id
                    )
                    if not operations:
                        move.bar_code_status = "nocompleted"
                    else:
                        if move.product_uom_qty == sum(
                ...
                for key in quantkey.keys():
                    ...
                    if key == "sinlote":
                        operations = pack_operation_ids.filtered(
                            lambda c: c.product_id.id == move.product_id.id
                            and not c.lot_id
                        )
                    else:
                        operations = pack_operation_ids.filtered(
                            lambda c: c.product_id.id == move.product_id.id
                            and c.lot_id
                            and c.lot_id.name == key
                        )
                    if not operations or (
                        operations and quantity != sum(operations.mapped("product_qty"))
                    ):
                        move.bar_code_status = "nocompleted"
                    else:
                        move.bar_code_status = "completed"
    ...
    def create(self, values):
        ...
        if "procurement_id" in values and values.get("procurement_id", False):
            proc = self.env["procurement.order"].browse(values.get("procurement_id"))
            if proc and proc.sale_line_id and proc.sale_line_id.notes2:
                values["notes2"] = proc.sale_line_id.notes2
        if "raw_material_production_id" in values and values.get(
            "raw_material_production_id", False
        ):
            production = production_obj.browse(values.get("raw_material_production_id"))
    ...
    def _get_invoice_line_vals(self, move, partner, inv_type):
        vals = super()._get_invoice_line_vals(move, partner, inv_type)
...
class StockPackOperation(models.Model):
    ...
    product_qty_lot = fields.Float(
        string="Quantity", digits_compute=dp.get_precision("Product Unit of Measure")
    )
    product_qty_in_lot = fields.Float(string="Total quantity in lot NOT RESERVED")
    product_qty_in_lot2 = fields.Float(string="Total quantity in lot NOT RESERVED")
    ...
    especial_lot = fields.Boolean(string="Lote especial", default=False)
    reserved_quant_for_especial_lot_ids = fields.Many2many(
        string="Reserved quant para lote especial",
    ...
    free_quant_for_especial_lot_ids = fields.Many2many(
        string="Free quant para lote especial",
        comodel_name="stock.quant",
        ...
        if self.product_lot_from_barcode:
            ...
            if lot:
                move = self.picking_id.move_lines.filtered(
                    ...
                    and lot.name in l.selected_lots
                )
            else:
                ...
                for m in moves:
                    for quant in m.reserved_quant_ids.filtered(
                        lambda l: l.product_id.id == product.id and not l.lot_id
                    ):
                        move = m
                        break
            if not move and not lot and product.type != "consu":
                raise exceptions.Warning(_("Product without lot not found in picking"))
            quants = False
            if lot and move:
            ...
            if lot and not move:
                ...
                qty_from_especial_lot = sum(qty_moves.mapped("product_uom_qty"))
                if "default_picking_id" in self.env.context:
                    ...
                    operations = picking.pack_operation_ids.filtered(
                        lambda l: l.product_id.id == product.id
                        and l.product_lot_from_barcode != self.product_lot_from_barcode
                    )
                    if operations:
                        qty_operations = sum(operations.mapped("product_qty"))
            ...
            if not quants and not lot and product.type != "consu":
                raise exceptions.Warning(_("Quant not fount for introduce product."))
            if (
                not quants
                and lot
                and not quants2
                and qty_from_especial_lot == pending_qty
            ):
                raise exceptions.Warning(
            ...
            if quants2:
                ...
                if especial_lot:
                    ...
                    if free_quants:
                        self.free_quant_for_especial_lot_ids = [(6, 0, free_quants.ids)]
            ...
            if (
                move
                and move.reserved_quant_ids
                and len(move.reserved_quant_ids) == 1
                and not lot
                and quants
                and not quants2
            ):
    ...
    def onchange_product_qty(self):
        for operation in self:
            if (
                operation.picking_id.purchase_subcontratacion
                and operation.product_qty > operation.product_qty_lot
                and operation.product_qty_lot > 0
            ):
    ...
    def onchange_lot_id(self):
        for operation in self.filtered(lambda x: x.lot_id):
            if (
                operation.picking_id.purchase_subcontratacion
                and operation.lot_id.quant_ids
            ):
                quants = operation.lot_id.mapped("quant_ids").filtered(
                    lambda l: l.product_id == operation.product_id
                    and l.qty == operation.product_qty
                ...
                if (
                    operation.picking_id.purchase_subcontratacion
                    and operation.product_id
                    and operation.product_id.type == "consu"
                ):
    ...
    def create(self, values):
        ...
        operation = super().create(values)
        if (
            operation.picking_id
            and not operation.picking_id.purchase_subcontratacion
            and not operation.date_expected
        ):
            move = operation.picking_id.move_lines.filtered(
                lambda x: x.product_id == operation.product_id
                and x.location_id == operation.location_id
                and x.location_dest_id == operation.location_dest_id
                and x.product_qty == operation.product_qty
            )
    ...
    def action_done(self):
        ...
        for inventory in self:
            if inventory.move_ids:
                inventory.move_ids.write({"inventory_description": inventory.name})
        return result
