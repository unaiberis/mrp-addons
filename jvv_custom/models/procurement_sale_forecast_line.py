...
class ProcurementSaleForecastLine(models.Model):
        ...
        for line in self:
            ...
            if line.partner_id:
                for supinf in line.product_tmpl_id.customer_ids.filtered(
                    lambda x: x.name == line.partner_id and
                        x.type == 'customer'):
    ...
    product_category_id = fields.Many2one(
        ...
        related='product_id.categ_id', store=True, readonly="1")
    product_tmpl_id = fields.Many2one(
        string='Product template', related="product_id.product_tmpl_id",
        store=True)
    product_code = fields.Char(
        string='Código producto para empresa', copy=False, store=True,
        compute='_compute_product_code', readonly="1")
    procurement_state = fields.Selection(
        string="Estado abastecimiento", store=True, copy=False,
        related="procurement_id.state")

    @api.multi
    def request_procurement(self):
        self.ensure_one()
        res = super(ProcurementSaleForecastLine, self).request_procurement()
    ...
    def create(self, values):
        line = super(ProcurementSaleForecastLine, self).create(values)
        line._create_forectas_lines2()
        return line

    def _create_forectas_lines2(self):
        vals = {'forecast_id': self.forecast_id.id,
                'date': self.date,
...
class ProcurementSaleForecastLine2(models.Model):
    ...
    requested_qty = fields.Float(
        string='Requested Quantity', default=0,
        digits_compute=dp.get_precision('Product Unit of Measure'))
    remaining_qty = fields.Float(
        string='Remaining Quantity', default=0,
        digits_compute=dp.get_precision('Product Unit of Measure'),
        readonly=True)
    completed_line = fields.Boolean(
        string="Completed line", default=False, readonly="1")
    sale_order_ids = fields.Many2many(
        string="In sale orders", comodel_name="sale.order", copy=False,
        readonly=True)
    ...
    def request_sale_order(self):
        ...
        if self.requested_qty > self.qty:
            raise exceptions.Warning(
                _("Requested quantity greater than quantity."))
        ...
        wiz = wiz_obj.with_context(
            {'active_id': self.id,
             'active_ids': self.ids,
             'active_model': 'procurement.sale.forecast.line2'}).create({})
        context = self.env.context.copy()
        context.update({
            'active_id': self.id,
            'active_ids': self.ids,
            ...
            if salelines:
                for saleline in salelines:
                    if saleline.order_id.state != 'draft':
                        raise exceptions.Warning(
                            _('You cannot delete this forecast line because it'
                              ' is associated with the sales order: %s, that is'
                              ' not in draft status.') %
                            (saleline.order_id.name))
                    if saleline not in sale_lines:
                        sale_lines += saleline
        result = super(ProcurementSaleForecastLine2, self).unlink()
