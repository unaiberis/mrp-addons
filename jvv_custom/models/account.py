...
class AccountInvoice(models.Model):
    ...
    def _compute_delivery_address_id(self):
        for invoice in self.filtered(lambda l: l.partner_id):
            partner = (invoice.partner_id if not invoice.partner_id.parent_id
                       else invoice.partner_id.parent_id)
            if partner.child_ids:
                p = partner.mapped('child_ids').filtered(
    ...
    sale_order_ids = fields.Many2many(
        comodel_name="sale.order", relation="rel_account_invoice_sale_order",
        column1="invoice_id", column2="sale_id", string="Sale orders",
        copy=False, compute='_compute_sale_order_ids', store=True)
    purchase_order_ids = fields.Many2many(
        comodel_name="purchase.order",
        relation="rel_account_invoice_purchase_order",
        column1="invoice_id", column2="purchase_id", string="Purchase orders",
        copy=False, compute='_compute_purchase_order_ids', store=True)
    ...
    mrp_repair_id = fields.Many2one(
        string='Repair order', comodel_name='mrp.repair',
        compute='_compute_mrp_repair_id', store=True, copy=False)
    crm_claim_id = fields.Many2one(
        string='Claim', comodel_name='crm.claim', store=True,
    ...
    def invoice_validate(self):
        for invoice in self.filtered(lambda l: l.reference_type and
                                     l.type == 'in_invoice'):
            if invoice.reference:
                cond = [('reference', '=', invoice.reference),
                        ('type', '=', 'in_invoice'),
                        ('id', '!=', invoice.id)]
                i = self.search(cond, limit=1)
                if i:
                    raise exceptions.Warning(
                        _("Payment Reference used in Supplier Invoice Number:"
                          " %s") % i.supplier_invoice_number)
...
class AccountInvoiceLine(models.Model):
    ...
    invoice_number = fields.Char(
        string='Invoice number', related='invoice_id.invoice_number',
        store=True)
    type = fields.Selection(related='invoice_id.type', store=True)
    partner_id = fields.Many2one(
        comodel_name='res.partner', related='invoice_id.partner_id',
        store=True)
