...
class PurchaseOrder(models.Model):
    ...
    mrp_production_product_id = fields.Many2one(
        ...
        related="mrp_production.product_id",
    ...
    dates_transfer_in_pickings = fields.Char(
        string="In pickings dates transfer",
        store=True,
        compute="_compute_dates_transfer_in_pickings",
        copy=False,
    )
    ...
    def wkf_confirm_order(self):
        ...
        res = super().wkf_confirm_order()
        for p in self.filtered(lambda c: c.state == "confirmed" and c.mrp_production):
            ...
            if p.mrp_operation.out_picking:
                ...
                if (
                    ...
                    and p.mrp_operation
                    and p.mrp_operation.out_picking
                ):
                ...
                for pi in pickings:
                    pi.production_qty = p.mrp_production.product_qty
                    if (
                        move
                        and move.raw_material_production_id
                        and p.mrp_operation
    ...
    def wkf_send_rfq(self):
        for purchase in self.filtered(lambda x: x.partner_id):
            ...
            if "@" not in purchase.partner_id.email:
                raise exceptions.Warning(
                    _("Partner %s with no recognizable email.") % self.partner_id.name
                )
    ...
    def write(self, values):
        if "mrp_production" in values and not values.get("mrp_production"):
            for purchase in self.filtered(lambda x: x.mrp_production):
                raise exceptions.Warning(
                    _(
                        "You are trying to unlink the purchase order %s from the"
    ...
    def copy(self, default=None):
        ...
        for purchase in purchases:
            for line in purchase.order_line:
                ...
                line.write(
                    {"name": name, "date_planned": fields.Date.context_today(self)}
                )
        return purchases
...
class PurchaseOrderLine(models.Model):
    ...
    def create(self, values):
        if (
            ...
            and values.get("product_id", False)
            and "virtual_availble" not in values
        ):
    ...
    def write(self, values):
        ...
        if (
            not self.env.context.get("no_call_wizard", False)
            and values.get("product_qty", False)
            and values.get("product_qty") > 0
        ):
            for line in self.filtered(
                lambda c: c.product_id.type == "product" and c.order_id.mrp_production
            ):
    ...
    def onchange_product_id(
        ...
        if product_id and partner_id:
            ...
            supplierinfo = p.supplier_ids.filtered(lambda x: x.name.id == partner_id)
            if (
                supplierinfo
                and supplierinfo.expiration_date
                and mydate >= supplierinfo.expiration_date
            ):
