...
class SaleOrder(models.Model):
    ...
    procurement_sale_forecast_ids = fields.Many2many(
        ...
        column1="sale_order_id", column2="forecast_id", copy=False,
        readonly=True)
    ...
    def create(self, vals):
        ...
        if ('origin' in vals and vals.get('origin', False) and
            ('Opportunity' in vals.get('origin', False) or
             'Oportunidad' in vals.get('origin', False))):
            if not partner:
                partner = partner_obj.browse(vals.get('partner_id'))
...
class SaleOrderLine(models.Model):
    ...
    @api.multi
    @api.depends('order_partner_id', 'product_id', 'product_id.customer_ids',
    ...
    def _compute_customer_product_code_with_deno(self):
        for line in self.filtered(lambda x: x.product_id and
                                  x.order_partner_id):
    ...
    out_picking_id = fields.Many2one(
        string='Out picking', comodel_name='stock.picking')
    out_picking_date_done = fields.Date(
        string='Transfer date out picking', store=True,
        compute='_compute_out_picking_date_done')
    pricelist_id = fields.Many2one(
        comodel_name='product.pricelist', string='Pricelist', store=True,
        readonly=True, related='order_id.pricelist_id', copy=False)
    currency_id = fields.Many2one(
        comodel_name='res.currency', string='Currency', store=True,
        readonly=True, related='pricelist_id.currency_id', copy=False)
    from_forecast_id = fields.Many2one(
        string="Created from sale forecast", comodel_name="procurement.sale.forecast",
    ...
    def product_id_change_with_wh(self, pricelist, product, qty=0,
        ...
        res = super(SaleOrderLine, self).product_id_change_with_wh(pricelist,
                product, qty=qty, uom=uom, qty_uos=qty_uos, uos=uos, name=name,
                partner_id=partner_id, lang=lang, update_tax=update_tax,
                date_order=date_order, packaging=packaging,
                fiscal_position=packaging, flag=flag,
                warehouse_id=warehouse_id)
        if product and partner_id:
            mydate = fields.Date.context_today(self)
            p = self.env['product.product'].browse(product)
            customerinfo = p.customer_ids.filtered(
                lambda x: x.name.id == partner_id)
            customerinfo2 = self.env['product.supplierinfo']
            for l in p.customer_ids.filtered(
                    lambda x: x.name.id == partner_id and x.delay and
                    x.date_validity):
                if (mydate >= l.date_validity and
                   (not l.expiration_date or l.expiration_date) >= mydate):
                    customerinfo2 += l
            if customerinfo2 and customerinfo2.round_quantity_sale:
                customerinfo = customerinfo2
            if customerinfo:
                res['value']['customer_product_code'] = (
                    customerinfo.product_code)
                if (customerinfo.expiration_date and
                        mydate >= customerinfo.expiration_date):
                    if ('warning' in res and
                        ...
                        warning = {'title': _('Price Error!'),
                                   'message' : _(
                                       'Product with expired rate.')}
                        res['warning'] = warning
            customerinfo2 = p.customer_ids.filtered(
                lambda x: x.name.id == partner_id and x.delay and
                x.date_validity and x.date_validity >= mydate and
                (not x.expiration_date or x.expiration_date <= mydate))
            if customerinfo2:
                value = res.get('value')
            ...
            if p.default_code:
                ...
                if name and p.default_code not in name:
                    res['value']['name'] = u"[{}] {}".format(
                        p.default_code, name)
                else:
                    res['value']['name'] = name
            cond = [('product_id', '=', p.id),
            ...
            if forecasts:
                ...
                for forecast in forecasts:
                    forecast_lit = (
                        forecast.name if not forecast_lit else
                        u"{}, {}".format(forecast_lit, forecast.name))
                message = u"El producto: {}, se encuentra en las siguientes " \
                ...
                if ('warning' in res and
                    ...
                    if "assemtronic" not in self.env.cr.dbname:
                        warning = {'title': 'Producto en previsiones',
                                   'message' : message}
                        res['warning'] = warning
            if (not forecasts and not p.include_in_forecast and
                    "assemtronic" not in self.env.cr.dbname):
                message = u"El producto: {}, no se incluye en previsiones".format(p.name)
                if ('warning' in res and
                        res.get('warning').get('message', False)):
                    if ('with expired rate' not in
                            res.get('warning').get('message')):
    ...
    def unlink(self):
        ...
        for line in self:
            if (line.from_forecast_line2_id and
                    line.from_forecast_line2_id not in forecast_lines):
    ...
    def forecast_lines_after_unlink_sale_order_line(self, forecast_lines):
        ...
        for forecast_line in forecast_lines:
            ...
            cond = [("from_forecast_line2_id", "!=", False),
            ...
            if sale_orders:
                ...
                forecast_line.write({'remaining_qty': 0,
                                     'completed_line': False})
        for forecast in forecasts:
            sale_orders = self.env['sale.order']
