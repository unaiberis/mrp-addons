...
class ProductSupplierinfo(models.Model):
    ...
    def _get_default_product_id(self):
        try:
            if ('default_product_tmpl_id' in self.env.context and
                    self.env.context.get('default_product_tmpl_id', False)):
    ...
    prefix_code = fields.Char(
        string='Ref. interna producto', related='product_tmpl_id.prefix_code',
        store=True)
    date_validity = fields.Date(string='Date validity')
    expiration_date = fields.Date(string='Expiration date')
    offer_number = fields.Char(string='Offer number')
    property_product_pricelist = fields.Many2one(
        string='Tarifa venta', comodel_name='product.pricelist',
    ...
    property_product_pricelist_purchase = fields.Many2one(
        string='Tarifa compra', comodel_name='product.pricelist',
    ...
    def onchange_date_validity(self):
        ...
        if self.date_validity:
            self.expiration_date = fields.Date.to_string(
                fields.Date.from_string(self.date_validity) +
