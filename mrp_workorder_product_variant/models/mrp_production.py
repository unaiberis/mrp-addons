...
class MrpProduction(models.Model):
    ...
    def _onchange_workorder_ids(self):
        result = super()._onchange_workorder_ids()
        if self.bom_id and self.workorder_ids:
            for workorder in self.workorder_ids:
                ...
                if (
                    ...
                    and (
                        workorder.operation_id.bom_product_template_attribute_value_ids
                    )
                    and (
                        not any(
                            [
                                line in attributes
                                for line in workorder.operation_id.bom_product_template_attribute_value_ids
                            ]
                        )
                    )
                ):
