...
class MrpProduction(models.Model):
    ...
    import_line_ids = fields.One2many(
        comodel_name="bizerba.import.line", inverse_name="production_id"
    )
    ...
    def action_conect_with_bizerba(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            query = (
                "SELECT * FROM [Pesada Individual] as p WHERE LEFT(p.GT1F,4)+'-'+SUBSTRING(p.GT1F,5,2)+'-'+SUBSTRING(p.GT1F,7,2)+' '+SUBSTRING(p.GT1F,10,2)+':'+RIGHT(p.GT1F,2)+':00' BETWEEN '"
                + str(start_date)
                + "' AND '"
                + str(stop_date)
                + "'"
